...
std::vector<int> MaxBandAdaptor::getFlow(Network* net, int start, int end)
{
 ...
 // visited.push_back(start);
  ...
  while(visited.size() < vertAmt)
  {
    ...
    if(std::get<1>(weights[cur]) == INT_MIN)
    {
  ...
  std::cout << "HIGHEST BANDWIDTH PATH: ";
  int cheapestNeighborWeight = INT_MIN;
  int bCur = end;
  int tempNodeID;
  while(bCur != start)
  {
...
int MaxBandAdaptor::chooseNextNode(std::vector<std::tuple<int, int> > weights, std::vector<int> visited){
   int nextNodeID;
   ...
   for(int i = 0; i < numNodes; i++)
   {
      ...
      if(std::get<1>(weights[i]) >= max && !(std::find(visited.begin(), visited.end(), i) != visited.end())) 
      {
        max = std::get<1>(weights[i]);
	      nextNodeID = i;
